...
-- customers 계속 쓰기 귀찮으니 c로 줄여버릴 수 있다.
    (
...
-- 더 많은 데이터를 가져오거나 코드가 길어질수록 같은 내용이 반복되고 귀찮고 실수할 확률도 커진다 --> JOIN
...
-- 아까 서브쿼리로 하던거보다 훨씬 쉽고 간결함

-- 모든 고객의 구매 현황 분석(구매를 하지 않았어도 분석하고 싶음)
SELECT
    *
FROM customers c
...
-- 조건 없이 붙이면 customers-sales를 진짜 모두 붙이기만 함(customers 50줄 * sales 120줄 = 6000줄)
...
-- 그런데 잘 보면, 이 테이블에서는 잠재고객을 절대 찾을 수 없다. -> 구매한 사람들만 대상으로 데이터를 정리했기 때문
