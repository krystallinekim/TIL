...
재귀함수를 이용할 때, 먼저 점화식을 만들어 보고 그걸 토대로 코드를 짜는 게 이해하기 편함

재귀함수는 무한루프 구조를 가지기 때문에 항상 base case를 만들어 줘서 끊어줘야 한다.

주피터 노트북에서는 자체적으로 1000줄에서 재귀를 끊어버린다.
...
def fact_while(n):
    ...
    return answer
```
재귀함수는 입력값이 클수록, 연산 속도가 길어진다.

이미 계산한 내용을 계속 다시 계산하기 때문에, 피보나치 수열 같은 경우 입력값이 하나 커질수록 연산량이 2배 조금 안되게 늘어나게 된다.

반면, 반복문은 계산값을 계속 덮어씌우며 연산해 계산량이 크게 늘어나지 않는다.

그래서 일반적으로는 반복문을 쓰지만, 알고리즘 자체가 재귀적 표현이 자연스러울 경우에는 재귀함수를 이용한다.
