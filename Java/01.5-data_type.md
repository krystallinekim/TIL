# 자료형

자료형은 데이터의 타입을 의미하며, 변수에 저장할 값의 형태에 맞게 지정해야 한다.

## 데이터 저장 단위

- 비트(bit)
    - 컴퓨터가 데이터를 표현하는 가장 기본적인 단위이다.
    - 0과 1의 2진수 값 하나를 저장할 수 있는 메모리 공간을 의미한다.
- 바이트(byte)
    - 데이터 처리 또는 문자의 최소 단위이다.
    - 8개의 비트가 모여 하나의 바이트가 된다.
    
- 이러한 데이터 저장 단위를 바탕으로, 자바는 다양한 자료형을 제공한다.

## 기본 타입

- 정수, 실수, 문자, 논리 타입을 기본 타입이라고 한다.
- 기본 타입은 정해진 메모리 크기(byte)를 사용해 값을 저장하는데 바이트 크기가 클수록 표현하는 값의 범위가 크다.
    
    | 타입 | 자료형 | 범위 | 크기(byte) | 기본값 |
    | --- | --- | --- | --- | --- |
    | 논리형 | boolean | true, false | 1 | false |
    | 문자형 | char | 0 ~ 65,535 (유니코드) | 2 | '\u0000' |
    | 정수형 | byte | -128 ~ 127 | 1 | 0 |
    | 정수형 | short | -32,768 ~ 32767 | 2 | 0 |
    | 정수형 | int | -2,147,483,648 ~ 2,147,483,647 | 4 | 0 |
    | 정수형 | long | -9,223,372,036,854,775,808 ~ 9,223,372,036,854,775,807 | 8 | 0L |
    | 실수형 | float | ±1.4E-45 ~ 3.4E38 | 4 | 0.0f |
    | 실수형 | double | ±4.9E-324 ~ 1.8E308 | 8 | 0.0 또는 0.0d |

- long 타입 숫자 뒤에는 대문자 L을 붙여 줘야 함
- float도 double과 구분하기 위해 F를 붙임
- 문자형(char)는 작은따옴표 사용함. 큰따옴표는 문자열

## 참조 타입

- 참조 타입은 값 자체가 아닌 **객체가 저장된 메모리 주소**를 참조하는 타입이다.
- 변수에는 실제 데이터가 아닌 객체의 참조값이 저장된다.
    ```java
    Variable variable = new Variable();  // variable
    ```
- 대충 클래스가 자료형으로 오는 변수는 다 참조타입 변수
- 클래스, 배열, 인터페이스 등이 참조 타입에 해당한다.
- 아무것도 참조하지 않는다는 의미로 null참조가 가능함.
    ```java
    String str = null;
    ```
    

### 문자열(String)

- 자바에서 큰 따옴표(”)로 감싼 단어나 문장을 문자열(String)이라고 한다.
- 문자열은 클래스 타입으로 문자열 타입의 변수는 참조 변수이다.
- 문자열은 참조 타입이지만(new 써서 만들어도 됨), 기본 타입처럼 사용할 수 있도록 특별히!! 지원된다.
- 기본타입이 아닌 이유는 실행될 때까지 크기 지정이 안되어 있기 때문 -> 메모리를 **동적 할당**함
    
    ```java
    String str = "오리";
    String str = new String("오리");
    String str = "오리" + "꽥꽥"; // 문자열 연결 연산
    String str = new String("오리" + "꽥꽥");
    ```

## 열거 타입

- 데이터 중에는 몇 가지로 한정된 값만 갖는 데이터가 있다.

- 한정된 값만을 갖는 데이터 타입이 열거 타입(Enumeration Type)이다.

- 열거 타입은 몇 개의 열거 상수(Enumeration Constant) 중에서 하나의 상수를 저장하는 데이터 타입이다.

- 열거 상수는 Enum 타입의 클래스에서 만들어진 객체처럼 동작함

### 열거 타입 선언

- 열거 타입은 클래스와 동일하게 소스 파일(.java)을 생성해야 한다.
    - 열거 상수는 모두 대문자로 작성하는 것이 관례이다.
    - 열거 타입의 이름은 소스 파일명과 동일해야 한다.
    
    ```java
    // enum은 열거 타입을 선언하기 위한 키워드이다.
    public enum Week {
        MONDAY,     // 0
        TUESDAY,    // 1
        WEDNESDAY,  // 2
        THURSDAY,   // 3
        FRIDAY,     // 4
        SATURDAY,   // 5
        SUNDAY      // 6
    }
    ```
    

### 열거 타입 변수

- 열거 타입도 하나의 데이터 타입이므로 변수를 선언하고 사용해야 한다. `자료형 변수명;`
    
    ```java
    Week today;
    ```
    
- 선언된 열거 타입 변수에 열거 상수를 저장할 수 있다.
- 열거 상수는 단독으로 사용할 수 없고 반드시 `열거 타입.열거 상수`로 사용된다.
    
    ```java
    Week today = Week.FRIDAY;
    ```
    

### 열거 타입 메소드

- 일반 메소드
    
    ```java
    System.out.println(today.name());  // FRIDAY
    System.out.println(today.ordinal());  // 4
    System.out.println(today.equals(Week.SUNDAY));  // false
    System.out.println(today.compareTo(Week.SUNDAY));  // -2
    ```
    - `.name()` 메소드는 열거 상수가 가지고 있는 문자열을 반환한다.
    - `.ordinal()` 메소드는 전체 열거 상수 중 몇 번째 열거 상수인지 정숫값을 반환한다. (0부터 시작)
    - `.equals()` 메소드는 매개값으로 전달되는 열거 상수와 동일한 열거 상수인지 확인 후 true 또는 false를 반환한다.
    - `.compareTo()` 메소드는 매개값으로 주어진 열거 상수를 기준으로 앞뒤로 몇 번째에 위치하는지 알려주는 정숫값을 반환한다.

- 열거타입에서 지원하는 정적 메소드들

    ```java
    Week friday = Week.valueOf("FRIDAY");
    System.out.println(friday);

    Week[] weekdays = Week.values();
    System.out.println(Arrays.toString(weekdays));
    ```

    - `.valueOf()` 메소드는 클래스에 적용되며, 문자열을 보고 문자열과 같은 타입/이름의 열거 상수(객체)를 찾아온다.
    - `.values()` 메소드는 열거 상수들을 배열에 저장해서 전달해준다.

### 열거 타입에 속성 추가하기

- 열거 타입의 열거 상수에 다른 속성을 추가할 수 있다.
    - 열거 상수의 괄호()를 넣고 추가로 할 속성을 적어준다.

- 객체에 속성을 추가할 때 필드를 이용하는 것처럼, 열거 타입에 필드를 설정하고 Getter를 통해 읽어 올 수 있다.
    - 열거타입의 필드는 final로 생성해서 만들 수 있다

- 열거 타입의 생성자는 반드시 private으로 선언해야 한다.
    - 열거상수 하나하나가 객체이므로, 생성자를 통해 각 필드 값을 초기화 해주는 것
    
    ```java
    public enum Week {
      MONDAY("monday", 1),
      TUESDAY("tuesday", 2),
      WEDNESDAY("wednesday", 3),
      THURSDAY("thursday", 4),
      FRIDAY("friday", 5),
      SATURDAY("saturday", 6),
      SUNDAY("sunday", 7);  // 열거상수만 있을때는 ; 필요없음
    
      private final String name;
      private final int value;
    
      private Week(String name, int value) {
          this.name = name;
          this.value = value;
      }
    
      // 열거 타입의 name 값을 읽어오는 메소드이다.
      public String getName() {
          return this.name;
      }
    
      // 열거 타입의 value 값을 읽어오는 메소드이다.
      public int getValue() {
          return this.value;
      }
    }
    ```


### 형 변환(Type Casting)

- 데이터의 타입을 변환하는 것을 형 변환(Type Casting)이라 한다. (boolean 제외)
- 자료형 간 표현 범위의 차이에 따라 자동 형 변환, 강제 형 변환으로 나뉜다.

    - 자동 형 변환은 **컴파일러가** 자동으로 값의 범위가 작은 자료형(int)의 값을 범위가 큰 자료형(double)으로 변환한다.

    - 강제 형 변환은 값의 표현 범위가 큰 자료형(double)을 작은 자료형(int)으로 강제로 변환한다.
    
    ```java
        // 자동 형 변환 (단, byte와 short 자료형 값의 계산 결과는 int로 처리한다.)
        double result = 12 + 3.3; // 12.0 + 3.3 = 15.3 (데이터 손실 없음)
    
        // 강제 형 변환
        int result = 12 + (int) 3.3; // 12 + 3 = 15 (0.3 데이터 손실 발생)

        System.out.println((byte) 290);  // 34
        // 290 = 100100010(2), 34 = 0100010(2) -> 이진수에서 표현할 수 있는 부분만 표현됨. 맨 앞자리가 1이면 음수도 나옴

    
    ```

- 나중엔 클래스 타입에서도 형변환이 일어남

 