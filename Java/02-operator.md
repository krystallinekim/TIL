...
- 일반적으로 수학에서는 `0 < num < 100` 처럼 사용함.
    - 실제로 코드로 실행될 때는, `0 < num`이 먼저 실행되고, 그 결과값인 `true`와 100 사이의 비교연산을 진행하게 됨.
    - boolean은 대소비교가 불가하므로 에러가 난다.
...
#### 단락회로평가 / 단축평가
논리 연산자를 사용 시 전체 결과가 이미 결정된 경우 나머지 피연산자는 실행하지 않고 넘어감
...
- `&&`(AND) 기준으로 false가 나오면 *(이미 false 확정)* 뒤쪽 연산은 굳이 수행하지 않음

- `||`(OR) 기준으로 true가 나오면 *(이미 true 확정)* 뒤쪽 연산은 수행하지 않는다.
...
- 삼항 연산자는 세 개의 피연산자를 필요로 하는 연산자이다. 연산기호도 2개(`?`, `:`)

- 삼항 연산자는 if 문으로 변경해서 작성할 수 있지만 한 줄에 간단하게 사용하고자 할 때 사용한다.
- 삼항 연산자는 중첩해서 여러 번 사용이 가능하다.(가독성 떨어져서 여러번 중첩할거면 그냥 if문 쓴다)
