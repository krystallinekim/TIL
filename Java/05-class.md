...
- default 접근 제한자는 *같은 패키지*에 소속된 클래스에서만 접근이 가능하다.
...
- 실제 Heap 영역에는 이 필드 값들이 메모리에 저장되고, 객체의 변수명에 그 주소가 저장된다.
    - 클래스가 자료형인 이유는 여기에 있음(사용자 정의 자료형)
...
- 생성자는 new 연산자로 호출되는 특별한 메소드로 클래스로부터 객체를 생성할 때 호출되어 객체의 **초기화를 담당**한다.
