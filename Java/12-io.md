...
- 보조 스트림은 반드시 기반 스트림(InputStream, OutputStream, Reader, Writer)을 먼저 생성한 후 이를 이용하여 보조 스트림을 생성해야 한다.

- 보조 스트림은 꼭 하나만 쓸 수 있는게 아니다.
    ```java
    InputStreamReader isr = new InputStreamReader(System.in);
    BufferedReader br = new BufferedReader(isr);
    ```
    - `System.in`이라는 기반 스트림에 문자 변환 보조 스트림을 붙이고, 성능 향상을 위해 보조 스트림을 하나 더 붙인 것
    - 인라인으로 한번에 적어도 된다.
...
`InputStreamReader`는 바이트 기반의 InputStream에 연결되어 문자 기반의 Reader로 변환하는 보조 스트림이다.
...
`OutputStreamWriter`는 바이트 기반의 OutputStream에 연결되어 문자 기반의 Writer로 변환하는 보조 스트림이다.
...
- 입출력 장치(하드디스크, 네트워크 등)와 직접 작업하지 않고 **버퍼(메모리)에 데이터를 모아 한 번에 입출력 작업**을 하여 실행 성능을 향상한다.
    - 기본적으로도 버퍼공간을 가지고 있지만, 그리 크진 않다.

- `BufferedOutputStream` 또는 `BufferedWriter`는 외부의 출력 장치로 데이터를 직접 출력하지 않고 버퍼에 쌓아두었다가 데이터를 한 번에 전송함으로써 출력 성능을 향상시키는 보조 스트림이다.
    ```java
    BufferedWriter bw = new BufferedWriter(new FileWriter("a_buffer.txt"));
    bw.write("안녕하세요");
    bw.newLine();  // 개행 메소드
    ```
    - 보조스트림의 생산자에 매개값으로 기반 스트림 객체를 전달해 주면 된다.

- `BufferedInputStream` 또는 `BufferedReader`는 외부의 입력 장치로부터 데이터를 직접 읽지 않고 버퍼에 쌓아두었다가 데이터를 한 번에 읽음으로써 읽기 성능을 향상시키는 보조 스트림이다.

    ```java
    BufferedReader br = new BufferedReader(new FileReader("a_buffer.txt"));
    Syste.out.println(br.readLine());  // 한 줄을 읽어올 수 있다.
    Syste.out.println(br.readLine());  // 읽어올 게 없으면 null을 반환한다. -> while에서 반복문을 돌릴 수 있다.
    ```
    - 굳이 배열에 담을 필요 없이 줄별로 받아올 수 있는 것
...
- 입출력 장치로 기본 자료형의 데이터를 읽고 출력하는 경우에 사용한다.
    ...
    ```java
    try (
        DataOutputStream dos = new DataOutputStream(new FileOutputStream("b_data.txt"));  // 여러 리소스를 try에 넣을 때는 ;로 구분
...
- `DataOutputStream`은 바이트 기반의 데이터를 기본 자료형의 데이터로 출력할 수 있는 보조 스트림이다.
    - `.write` 뒤에 다양한 기본타입을 제공한다. `.writeInt()`(정수), `,writeUTF()`(문자열), `.writeDouble()`(실수), `.writeBoolean()`(논리값) 등등

- `DataInputStream`은 바이트 기반의 데이터를 기본 자료형의 데이터로 읽을 수 있는 보조 스트림이다.
    - 단, 입력된 자료형의 순서와 출력될 자료형의 순서가 일치해야 한다.
    - 다른 순서로 가져오면 원하지 않는 데이터가 튀어나올 것

### 객체 입출력 보조 스트림

- 입출력 장치로 **객체**를 읽고 출력하는 경우에 사용한다.
    - 객체의 필드 값들을 나열해 준다고 생각하면 된다.

- 단, 객체는 문자 데이터가 아니므로, 출력하거나 저장하려면 바이트 형태로 직렬화해야 한다.
    - **직렬화**란 스트림을 통해서 객체를 출력하기 위해 연속적인 데이터로 변환하는 것을 말한다.
    - **역직렬화**란 스트림을 통해서 읽어들인 연속적인 데이터를 객체로 복원하는 것을 말한다.
    - 이 둘을 하기 위해서는 `Serializable`이라는 인터페이스를 구현해 주어야 한다.
        - 딱히 이 인터페이스에서 요구하는 건 없고, 마킹을 위한 인터페이스


- `ObjectOutputStream`은 OutputStream에 연결되어 객체를 직렬화하는 보조 스트림이다.
    ```java
    ...
    ```
    - Serial 버전 UID가 있어야 input, output 시 서로 같은 클래스라고 인식하고 직렬화/역직렬화를 해준다. 

    - 프로그램이 가진 객체를 직렬화해서 출력 스트림으로 내보냄

- `ObjectInputStream`은 InputStream에 연결되어 객체를 역직렬화하는 보조 스트림이다.

    ```java
    ObjectInputStream ois = new ObjectInputStream(new FileInputStream("c_object.dat"))
    Member member = (Member) ois.readObject();
    System.out.println(member.toString());  // Member{id='Hong123', password='1234', name='홍길동', age=34, gender=M, height=179.9}
    ```

    - 직렬화된 데이터를 프로그램이 읽을 수 있게 읽어오는 역할
    - `.readObject()`의 반환타입은 Object이기 때문에, 다운캐스팅을 해서 받아와야 한다.

- 특정 데이터를 직렬화에서 제외하고 싶다면, 필드 생성 시 `transient` 키워드를 붙여 주면 된다.
    ```java
    public class Member implements Serializable {
    ...
    ```
    - 데이터 직렬화 시 이 키워드가 붙어 있다면 직렬화하지 않고 넘어가 기본값으로 채워놓는다.
