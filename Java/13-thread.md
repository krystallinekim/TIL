...
- `.join()` 메소드를 호출한 스레드(메인 스레드)는 메소드 앞에 붙어있는 스레드가 종료될 때까지 기다렸다가, 그 스레드가 종료되면 마저 실행한다.
...
- 스레드의 개수가 CPU 코어의 수보다 많을 경우, 스레드를 어떤 순서에 의해서 동시성으로 실행할 것인가를 결정하는 것을 스레드 스케줄링이라고 한다.
    - 스레드 4개를 돌리면 코어 4개에서 알아서 돌아감
    - 요즘은 CPU 성능이 좋아서 직접 확인하기는 힘들다

- **동시성**은 하나의 CPU 코어에서 멀티 스레드가 번갈아가며 실행하는 성질을 말한다.
    - 동시에 실행되는것처럼 보인다

- **병렬성**은 CPU 멀티 코어에서 개별 스레드를 동시에 실행하는 성질을 말한다.
...
- Thread 클래스의 `.setPriority()` 메소드를 사용해서 우선순위를 지정할 수 있다.
    - 상수로 MIN_PRIORITY, MAX_PRIORITY가 제공된다.
...
- 스레드의 상태를 제어하는 대표적인 메소드는 아래와 같다.
    ...
    - 여기 적힌 메소드들의 리턴 타입은 전부 `void`로, 호출만 하면 된다  
    - `wait()`에서 잠금은 동기화 상태일 때 잠그는 걸 의미

## 동기화(Synchronized)

- 원래 스레드는 독립적으로 작동하지만, Heap 영역의 객체를 공유하면서 작동한다.
    - 스레드가 사용 중인 객체의 작업이 끝날 때까지 사용 중인 객체에 잠금을 걸어서 다른 스레드가 접근할 수 없도록(스레드 작업이 끝날 때까지) 한다.

- 자바는 동기화를 위해 동기화 메소드와 동기화 블록을 제공하는데 스레드가 객체 내부에 **동기화 메소드** 또는 **동기화 블록**을 실행하면 즉시 객체에 잠금을 걸어 다른 스레드가 객체에 접근하지 못하도록 한다.

- 동기화 메소드와 동기화 블록이 객체 내에 여러 개 있을 경우 스레드가 이들 중 하나를 실행하면 다른 스레드는 해당 메소드는 물론이고 객체 내의 다른 동기화 메소드 및 동기화 블록도 실행할 수 없다.

    - 일반 메소드에는 접근 가능
    
    ![image.png](https://velog.velcdn.com/images/dms873/post/b067272a-b904-499c-8774-ab4d6bc77d25/image.png)
    
- 동기화 메소드는 선언부에 `synchronized` 키워드를 붙이면 된다.
    - 정적 메소드도 동기화 메소드로 만들 수 있다.
    ...
    public class Storage {
        ...
        public synchronized void getProduct() throws InterruptedException {
            ...
            // 소비했으니 isEmpty = true
            this.isEmpty = true;
            notify();  // wait에 의해 정지된 스레드 중 하나(Producer)를 실행 대기 상태로 만든다.
        }
        ...
        public synchronized void setProduct(int product) throws InterruptedException {
            ...
            // 생산했으니 isEmpty = false
            this.isEmpty = false;
            notify();  // wait에 의해 일시정지된 스레드 중 하나(Consumer)를 실행 대기 상태로 만든다.
        }
    }
    ```
    - 번갈아 가며 스레드가 작업하는 예시
    - 동기화로 자원은 효율적으로 사용할 수 있지만, 고려해야 될 것이 너무 많다.
    
- 메소드 전체 내용이 아니라 일부 내용만 동기화하고 싶을 때는 동기화 블록을 만들면 된다.
    - 메소드가 너무 길어서 메소드 전체를 기다리기 힘들 경우

    ```java
    public void 메소드명() {
        ...
    
        // 괄호() 안에는 잠글 객체를 지정하는데 보통 this를 지정한다.
        syschronized(this) {...}  // 이 작업이 끝날 때까지 다른 스레드에서는 접근 불가
    }
    ```