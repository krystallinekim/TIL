...
단, '100,000,000'같은 str의 경우 int로 변환 시 변환 가능한 부분(100)까지만 변환해줌.

또, 모든 타입에 대해 형변환이 가능한 것도 아님(`YEAR`, `TINYINT` 등등)

### 묵시적 형변환

산술 연산자 (`+`, `-`, `*`, `/` 등)의 경우, 계산시 숫자형 데이터가 아니면 알아서 형변환하고 계산함
