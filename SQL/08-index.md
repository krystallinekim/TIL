...
- `클러스터형 인덱스(Clustered Index)`: 테이블당 한 개만 존재하며, 일반적으로 `PRIMARY KEY`가 이에 해당한다.
   ...
   - 기본적으로 이 기준으로 원본 데이터를 오름차순으로 정렬해서 저장됨
   - 클러스터형 인덱스 기준 정렬 기준값(1000은 a페이지, 1001은 b페이지,....)이 저장된 테이블이 생긴다.
   - 검색 속도가 빠르지만, 데이터가 새로 추가되면 전체 인덱스를 다시 구성해야 한다

- `보조 인덱스(Secondary Index)`:  테이블당 여러 개를 생성할 수 있다. 

   - 클러스터형 인덱스 말고 전부 해당
   - 원본 데이터를 정렬하지 않는다. 데이터의 위치(a페이지의 b행)를 저장한 테이블이 생김
   - 데이터가 추가되어도 인덱스를 새로 구성할 필요가 없다
- 대신 속도가 느림

- `혼합형 인덱스`: 보통 인덱스를 하나만 지정하는 게 아니라, A열에 클러스터, B열에 보조 인덱스 이런 식으로 지정함
- 클러스터형 인덱스는 그대로 저장됨
   - 보조 인덱스는 위치를 저장하는 것이 아니라, 클러스터형 인덱스 값이 저장됨
   - 검색 시에는 약간 손해보지만, DML에서 보조 인덱스 파트를 새로 구성할 필요가 없다
