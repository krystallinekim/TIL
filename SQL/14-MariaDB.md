...
트리거는 테이블에 삽입, 수정, 삭제 등(DML)의 작업(이벤트)이 발생할 때 자동으로 작동되는 개체이다.

- 트리거는 트랜잭션 처리가 어렵다. DB에 반영된 후에 롤백하기 힘듦
- 백엔드에서 비즈니스 로직으로 짬
...
FOR EACH ROW  -- 영향받은 행마다 실행(없으면 업데이트쿼리 1번에 대해 1번만 실행)
BEGIN
    -- SQL 프로그래밍 코드 작성
END $$
DELIMITER ;
...
- `NEW`: `INSERT`시 새로 입력된 값이 들어있는 임시테이블 -> `NEW.pcode`처럼 값에 접근 가능
- `OLD`: `UPDATE`, `DELETE` 시 변경되거나 삭제된 값이 들어간 임시테이블
