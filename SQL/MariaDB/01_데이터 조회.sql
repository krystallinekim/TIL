...
SELECT *
FROM usertbl
-- NULL은 비교 연산자로 비교 불가능
-- WHERE mobile1 = NULL
-- NULL은 IS [NOT] NULL 연산자로 비교해야 한다.
...
-- 키가 182 이상인 회원의 아이디와 이름, 출생년도, 키 조회
SELECT `userid`,
		 `name`,
		 `birthYear`,
		 `height`
...
-- 급여가 5000000만원 이상인 직원들의 직원명, 부서 코드, 급여 조회
 SELECT emp_name, 
 		  dept_code,
 		  salary
 FROM employee
...
-- WHERE `addr` != '경남' AND `addr` != '경북' AND `addr` != '전남';
-- WHERE NOT (`addr` = '경남' OR `addr` = '경북' OR `addr` = '전남');
...
-- employee 테이블에서 부서별 급여의 합계를 조회 (부서별 오름차순으로 정렬)
SELECT dept_code, 
       SUM(salary)
FROM employee
GROUP BY dept_code
...
-- usertbl 테이블에서 가장 작은 키와 큰 키를 조회
SELECT MIN(height),
		 MAX(height)
FROM usertbl;
...
-- 즉, mobile1 열에 NULL인 것은 제외하고 카운트한다.
...
-- 조하은님
SELECT IFNULL(dept_code, '부서없음') AS '부서 코드', 
       COUNT(*) AS '부서별 사원의 수'
FROM employee
GROUP BY dept_code
;
...
-- HAVING 실습
-- buytbl 테이블에서 총 구매 금액이 1,000,000원 이상인 회원의 아이디, 총 구매 금액 조회
SELECT userid,
       SUM(price * amount)
FROM buytbl
-- 집계 함수의 결과를 WHERE 절에서 조건으로 사용할 수 없다.
-- WHERE SUM(price * amount) >= 1000000
