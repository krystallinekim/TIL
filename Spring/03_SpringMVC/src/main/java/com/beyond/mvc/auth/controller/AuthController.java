...
public class AuthController {
    ...
    @PostMapping("/auth/login/requestparam")
    ...
    @PostMapping("/auth/login/requestparam/attributes")
    public String login(
    ...
    @PostMapping("/auth/login/requestparam/sameparameters")
    public String login(
            @RequestParam String userName,
            @RequestParam String password,
    ...
    // @GetMapping에서 URL에 {}로 변수명 지정 -> @PathVariable로 변수의 값을 가져올 수 있다.
    @GetMapping("/auth/users/{no}")  // 중괄호 안의 값을 경로변수 no로 전달하겠다는 의미
    public String users(@PathVariable String no) {  // @PathVariable("no")가 일반적이지만, 이름이 같을 경우 생략
    // 어노테이션 자체를 생략하면 request parameter로 인식
        log.info("userNo: {}", no);
